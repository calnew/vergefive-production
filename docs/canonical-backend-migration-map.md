...
- The reviewed SQL in `schema/member-progress.sql` is applied in CI before a dev
  Worker deployment. Request handlers must not create or alter tables.
...
| Fix status | `readiness_signals` using `fix_progress` and `fix_done` |
