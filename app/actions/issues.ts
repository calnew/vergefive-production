...
export async function updateIssueStatus(issueId: string, status: "todo" | "progress" | "done", redirectTo?: string) {
  ...
  const updatedIssues = issue.scan.issues.map((item) => (item.id === issue.id ? { ...item, status } : item));
  const completedPoints = updatedIssues.filter((item) => item.status === "done").reduce((sum, item) => sum + (issuePointMap[item.key] ?? 4), 0);
  const score = Math.min(100, Math.max(issue.scan.readinessScore, 56) + completedPoints);
  const clean = Math.min(issue.scan.signalsTotal, Math.max(issue.scan.signalsClean, 5) + updatedIssues.filter((item) => item.status === "done").length);

  await prisma.scan.update({ where: { id: issue.scanId }, data: { readinessScore: score, grade: gradeForScore(score), signalsClean: clean } });

  const completedKeys = new Set(updatedIssues.filter((item) => item.status === "done").map((item) => item.key));
  const unlockNames = issue.scan.accountMatches.filter((match) => {
