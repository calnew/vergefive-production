(function(){
  ...
  function initFeedbackForm(){
    ...
    form.addEventListener('submit',function(event){
      ...
      var payload={
        name:(form.elements.name||{}).value||'',
        email:(form.elements.email||{}).value||'',
        type:(form.elements.type||{}).value||'Report a bug',
        severity:(form.elements.severity||{}).value||'Normal',
        pageUrl:(form.elements.pageUrl||{}).value||location.href,
        message:(form.elements.message||{}).value||'',
      ...
      fetch('/api/feedback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)})
        .then(function(res){return res.json().then(function(data){if(!res.ok)throw new Error(data.error||'Report could not be sent.');return data})})
        .then(function(){
          form.reset();
          if(pageUrl)pageUrl.value=location.href;
          if(status)status.textContent='Report sent. Thank you - we will review it.';
        })
        .catch(function(err){
          if(status){status.textContent=err.message||'Report could not be sent. Please try again.';status.classList.add('danger')}
        });
    });
  }
  initFeedbackForm();
  ...
  function initFeedbackBugButton(){
    ...
    style.textContent='.feedback-trigger{position:fixed;right:18px;bottom:72px;z-index:70;border:1px solid #7dd9e7;background:#071733;color:#fff;border-radius:999px;padding:10px 14px;font-size:13px;font-weight:900;text-decoration:none;box-shadow:0 14px 36px rgba(7,23,51,.18)}.feedback-trigger:hover{background:#0b3558}@media(max-width:760px){.feedback-trigger{right:12px;bottom:66px}}';
    document.head.appendChild(style);
    var a=document.createElement('a');
    a.className='feedback-trigger';
    a.href='/feedback/?from='+encodeURIComponent(location.pathname+location.search);
    a.setAttribute('data-feedback-trigger','');
    a.textContent='Report bug';
  ...
  function initAdminBackend(){
    ...
    function stalledBusinessCreditTemplate(member){
      ...
      if(!form)return;
      var data=template==='test-drive-welcome'?testDriveWelcomeTemplate(member):template==='stalled-business-credit'?stalledBusinessCreditTemplate(member):null;
      if(!data)return;
    ...
    function renderMemberDetail(data){
      ...
      detailBody.innerHTML='<div class="admin-detail-head"><div><strong>'+escapeHtml(m.name||'Member')+'</strong><span>'+escapeHtml(m.email||'')+'</span><small>'+escapeHtml(m.id||'')+'</small></div><span class="match-status ready">'+escapeHtml(m.membership_status||'none')+'</span></div>'+
        ...
        '<div class="admin-detail-section"><h3>Email member</h3><form data-admin-email-form class="admin-email-form"><label>Template<select class="input" data-admin-email-template><option value="custom">Custom message</option><option value="test-drive-welcome">Welcome / test drive invite</option><option value="stalled-business-credit">Stalled business credit invite</option></select></label><input class="input" name="subject" placeholder="Subject" value="Update from Verge Five"><textarea class="input" name="message" rows="5" placeholder="Write the message to '+escapeHtml(m.email||'this member')+'"></textarea><div class="proof-actions"><button class="btn" type="submit">Send email</button><button class="btn secondary" type="button" data-admin-load-template="test-drive-welcome">Load welcome template</button><button class="btn secondary" type="button" data-admin-load-template="stalled-business-credit">Load test drive invite</button><a class="btn secondary" href="mailto:'+encodeURIComponent(m.email||'')+'">Open email app</a></div><p class="legal" data-admin-email-status></p></form></div>'+
    ...
    if(detailBody)detailBody.addEventListener('submit',function(e){
      ...
      if(emailForm&&currentMemberId){
        ...
        postAdminAction({action:'send-email',memberId:currentMemberId,subject:(emailForm.elements.subject||{}).value||'',message:(emailForm.elements.message||{}).value||''})
