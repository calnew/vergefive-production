(function(){
  ...
  function upgradeHomepageConversion(){
    ...
    if(heroCopy)heroCopy.textContent='Built from the framework behind The Hidden Gatekeepers of Business Approval, Verge Five helps you see whether your business looks real, consistent, and ready before applications go out.';
    ...
    if(hero&&!document.querySelector('.home-ai-audit')){
      ...
      auditTop.innerHTML="<div class='section'><div class='ai-audit-card'><div><p class='kicker'>Free business visibility check</p><h2>Check what the public can find before you build.</h2><p>Use the same idea behind The Hidden Gatekeepers: first see how the business appears from the outside, then fix weak phone, address, website, email, and public-record signals before applying.</p><div class='mini-list'><span>Business name visibility</span><span>Public evidence and red flags</span><span>Before-and-after buildout comparison</span></div></div><a class='btn dark' href='/whats-inside/#quick-test-drive'>Start sample check</a></div></div>";
  ...
  function applyHiddenGatekeepersFraming(){
    ...
    if(path==='/whats-inside/'){
      ...
      setText('.whats-inside-hero .kicker','Based on The Hidden Gatekeepers');
      setText('.whats-inside-hero .page-title','The book explains the hidden approval system. Verge Five turns it into the buildout.');
      setText('.whats-inside-hero .page-sub','The Hidden Gatekeepers of Business Approval explains how automated systems read business identity, consistency, timing, and risk signals before a human ever reviews the file. Verge Five was created to turn that framework into a practical step-by-step platform: scan first, fix the foundation, then move toward vendors, credit cards, and funding only when the business profile is ready.');
  ...
  function initAdminBackend(){
    ...
    function addLegacyCampaignPanel(){
      ...
      function selectedLeadIds(){
        return [].slice.call(section.querySelectorAll('[data-legacy-lead-check]:checked')).map(function(input){return input.value});
      }
      function renderLegacy(){
        var q=(searchInput&&searchInput.value||'').toLowerCase();
        var filtered=leads.filter(function(lead){return !q||(lead.email+' '+leadFullName(lead)+' '+(lead.status||'')).toLowerCase().indexOf(q)>-1});
        if(metrics){
