(function(){
  ...
  function initAdminBackend(){
    ...
    function applyAdminEmailTemplate(form,member,template){
      if(!form)return;
      var data=template==='test-drive-welcome'?testDriveWelcomeTemplate(member):null;
      if(!data)return;
    ...
    function renderMemberDetail(data){
      ...
      detailBody.innerHTML='<div class="admin-detail-head"><div><strong>'+escapeHtml(m.name||'Member')+'</strong><span>'+escapeHtml(m.email||'')+'</span><small>'+escapeHtml(m.id||'')+'</small></div><span class="match-status ready">'+escapeHtml(m.membership_status||'none')+'</span></div>'+
        ...
        '<div class="admin-detail-section"><h3>Email member</h3><form data-admin-email-form class="admin-email-form"><label>Template<select class="input" data-admin-email-template><option value="custom">Custom message</option><option value="test-drive-welcome">Welcome / test drive invite</option></select></label><input class="input" name="subject" placeholder="Subject" value="Update from Verge Five"><textarea class="input" name="message" rows="5" placeholder="Write the message to '+escapeHtml(m.email||'this member')+'"></textarea><div class="proof-actions"><button class="btn" type="submit">Send email</button><button class="btn secondary" type="button" data-admin-load-template="test-drive-welcome">Load welcome template</button><a class="btn secondary" href="mailto:'+encodeURIComponent(m.email||'')+'">Open email app</a></div><p class="legal" data-admin-email-status></p></form></div>'+
