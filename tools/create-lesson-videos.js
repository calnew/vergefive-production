...
function pageHtml(lesson, audioBase64) {
...
function render(t){
  ...
  ctx.globalAlpha=.12;ctx.fillStyle='#22d3ee';ctx.beginPath();ctx.arc(1030+Math.sin(t*.28)*18,104,230,0,Math.PI*2);ctx.fill();ctx.fillStyle='#c49b45';ctx.beginPath();ctx.arc(1085,658,245,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;
  ...
  lesson.checks.forEach((item,i)=>{const y=232+i*48;ctx.fillStyle=i<=index?'#e7f5ee':'#eef3f8';rr(916,y,184,34,8);ctx.fill();ctx.fillStyle=i<=index?'#16825f':'#5b6473';ctx.font='800 15px Arial, sans-serif';ctx.fillText((i<=index?'✓ ':'• ')+item,930,y+23)});
...
(async () => {
  ...
  for (const lesson of selected) {
    ...
    await page.setContent(pageHtml(lesson, audioBase64), { waitUntil: 'load' });
