...
export async function onRequestPost(context) {
  ...
  let body = {};
  try {
    body = await readJson(context.request, 16 * 1024);
  } catch (error) {
  ...
  const type = cleanLimited(body.type, 40) || 'Report a bug';
  ...
  const formatted = [
    ...
    `Name: ${name}`,
    `Email: ${email}`,
    auth && auth.user ? `Member user id: ${auth.user.id}` : 'Member user id: Not logged in',
